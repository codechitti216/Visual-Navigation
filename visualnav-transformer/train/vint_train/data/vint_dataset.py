...
class ViNT_Dataset(Dataset):
    ...
    def _build_caches(self, use_tqdm: bool = True):
        ...
        for traj_name in self.traj_names:
            ...
            if not os.path.exists(traj_data_path):
                print(f"[DEBUG] Skipping {traj_name}: traj_data.pkl not found at {traj_data_path}")
                continue
            with open(traj_data_path, "rb") as f:
                traj_data = pickle.load(f)
            images = sorted([img for img in os.listdir(traj_folder) if img.endswith(".jpg")])
            num_images = len(images)
            pos_length = len(traj_data['position']) if 'position' in traj_data else 'N/A'
            print(f"[DEBUG] Trajectory '{traj_name}': {num_images} images, position length: {pos_length}")
            if num_images == 0:
                print(f"[DEBUG] Skipping {traj_name}: No .jpg images found in {traj_folder}")
                continue
            # Check if trajectory is long enough for context and prediction
            min_traj_len = self.context_size * self.waypoint_spacing + self.len_traj_pred * self.waypoint_spacing + self.end_slack + 1
            if isinstance(pos_length, int) and pos_length < min_traj_len:
                print(f"[DEBUG] Skipping {traj_name}: Trajectory too short (length {pos_length}, required {min_traj_len})")
                continue
            self.trajs.append({
                "traj_name": traj_name,
                "traj_data": traj_data,
        ...
        print(f"[DEBUG] Loaded {len(self.trajs)} trajectories for split '{self.dataset_name}' (expected from {len(self.traj_names)} names)")
        ...
        if not os.path.exists(cache_filename):
            ...
            with lmdb.open(cache_filename, map_size=2**32) as image_cache:
                with image_cache.begin(write=True) as txn:
                    for traj_name, time in tqdm_iterator:
                        image_path = get_data_path(self.data_folder, traj_name, time)
                        if not os.path.exists(image_path):
                            print(f"[LMDB BUILD] Missing image: {image_path}")
                            continue
    ...
    def _build_index(self, use_tqdm: bool = False):
        ...
        for traj_name in tqdm.tqdm(self.traj_names, disable=not use_tqdm, dynamic_ncols=True):
            ...
            print(f"[DEBUG] _build_index: traj_name={traj_name}, traj_len={traj_len}")
            ...
            print(f"[DEBUG] _build_index: begin_time={begin_time}, end_time={end_time}")
            count = 0
            for curr_time in range(begin_time, end_time):
                ...
                count += 1
            print(f"[DEBUG] _build_index: Added {count} samples for {traj_name}")

        print(f"[DEBUG] _build_index: Total samples_index={len(samples_index)}")
    ...
    def _load_image(self, trajectory_name, time):
        ...
        try:
            ...
            with self._image_cache.begin() as txn:
                image_buffer = txn.get(key.encode())
                if image_buffer is None:
                    print(f"[DEBUG][load_image] Image not found in LMDB: {key}")
                    raise FileNotFoundError(f"Image not found in LMDB: {key}")
                image_bytes = bytes(image_buffer)
            image_bytes = io.BytesIO(image_bytes)
            return img_path_to_data(image_bytes, self.image_size)
        except Exception as e:
            print(f"[DEBUG][load_image] Failed to load image {image_path}: {e}")
            raise
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        try:
            ...
            print(f"[DEBUG][__getitem__] context={context}")
            obs_imgs = []
            for f, t in context:
                img = self._load_image(f, t)
                if img is None:
                    print(f"[DEBUG][__getitem__] Image is None for (f={f}, t={t})")
                    raise RuntimeError(f"Image is None for (f={f}, t={t})")
                obs_imgs.append(img)
            obs_image = torch.cat(obs_imgs)
            ...
            print(f"[DEBUG][__getitem__] Skipping sample {i}: {e}")
            raise
