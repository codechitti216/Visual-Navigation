...
def main(config):
        ...
        for data_split_type in ["train", "test"]:
            if data_split_type in data_config:
                    ...
                    if data_split_type == "train":
                        ...
                        print(f"[DEBUG] Train dataset '{dataset_name}' size: {len(dataset)}")
                    else:
                        dataset_type = f"{dataset_name}_{data_split_type}"
                        if dataset_type not in test_dataloaders:
                            test_dataloaders[dataset_type] = {}
                        test_dataloaders[dataset_type] = dataset
                        print(f"[DEBUG] Test dataset '{dataset_type}' size: {len(dataset)}")
    ...
    if config["model_type"] == "gnm":
        ...
        checkpoint_path = "../pretrained/vint.pth"
        if os.path.exists(checkpoint_path):
            checkpoint = torch.load(checkpoint_path, map_location="cpu")
            load_model(model, "vint", checkpoint)
